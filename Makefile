.PHONY: compute render clean-cache notebook help check-manuscript \
...
# ── Manuscript regression test ────────────────────────────────────────────────
# Execute every {python} block in tas_manuscript.qmd headlessly; verify every
# cross-reference resolves and every citation exists in bibliography.bib; and
# regenerate output/stats.json (scripts/extract_stats.py) to fail loudly if any
# quoted prose statistic has drifted from the current output files.
# Run before every `quarto render tas_manuscript.qmd`.
check-manuscript:
