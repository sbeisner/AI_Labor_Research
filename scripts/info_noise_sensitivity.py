"""Managerial-information noise robustness sweep.

Adds Gaussian observation error to the firm's read of each worker's
`r_job` (substitution risk) and `p_aug` (augmentation potential) at every
layoff and matching decision. Defends the model against the peer-review
critique that headline findings depend on the assumption of perfect
managerial information about an employee's AI augmentability.

Sweeps σ_obs ∈ {0.0, 0.1, 0.2} (applied symmetrically to both r_job and
p_aug) × N_RUNS paired seeds. Output: output/info_noise_sensitivity.parquet
with columns matching paired_runs.parquet plus a leading `obs_noise` column.

This is exploratory (50 seeds, not the headline 100) — sufficient to show
qualitative invariance of the headline result.
"""
...
DEFAULT_GRID = (0.0, 0.1, 0.2)
N_RUNS       = 50
N_TICKS      = 180
BURN_IN      = 60
OUT_PATH     = ROOT / "output" / "info_noise_sensitivity.parquet"
...
def run_seed(args: tuple[int, float]) -> pd.DataFrame:
    ...
    params["p_aug_noise"] = float(obs_noise)
    params["r_job_noise"] = float(obs_noise)
...
def main() -> int:
    ...
    p.add_argument("--grid", default=None,
                   help=f"comma-separated σ_obs values (default: {','.join(map(str, DEFAULT_GRID))})")
    ...
    print(
        f"[info_noise_sensitivity] σ_obs grid: {list(grid)}\n"
        f"  paired seeds per σ_obs: {n_runs}\n"
    ...
    tasks = [(seed, obs) for obs in grid for seed in range(n_runs)]
    total = len(tasks)
    ...
    print(f"\n[info_noise_sensitivity] Saved {len(out_df)} rows to {out_path}", flush=True)
    return 0
